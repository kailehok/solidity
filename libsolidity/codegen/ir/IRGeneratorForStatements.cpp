...
void IRGeneratorForStatements::endVisit(FunctionCall const& _functionCall)
{
	...
	{
		FunctionDefinition const* unresolvedFunctionDef = IRHelpers::referencedFunctionDeclaration(_functionCall.expression());

		solAssert((memberAccess && unresolvedFunctionDef) || !functionType->bound(), "");
		...
		vector<string> args;
		if (memberAccess && functionType->bound())
		{
			solAssert(functionDef->parameters().size() == arguments.size() + 1, "");
			if (functionType->takesArbitraryParameters())
				args += IRVariable(memberAccess->expression()).stackSlots();
			else
