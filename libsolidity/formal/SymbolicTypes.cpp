...
SortPointer smtSort(frontend::Type const& _type)
{
	...
	case Kind::Array:
	{
		...
		else if (isStringLiteral(_type.category()))
		{
			...
			return make_shared<ArraySort>(SortProvider::uintSort, SortProvider::uintSort);
		}
		else
		{
			...
			solAssert(arrayType, "");
			return make_shared<ArraySort>(SortProvider::uintSort, smtSortAbstractFunction(*arrayType->baseType()));
		}
	}
	case Kind::Tuple:
	{
